 ...
 * ======================================================= */
function setupPrototypeWarning() {
    const overlay = document.getElementById('prototype-warning-overlay');
    const closeBtn = document.getElementById('prototype-warning-close');

    if (!overlay || !closeBtn) {
        console.error("ERRO: Elementos do Modal de Aviso de Protótipo não encontrados.");
        return;
    }

    // 1. Verifica se o usuário JÁ VIU o aviso nesta sessão
    const hasSeenWarning = sessionStorage.getItem('seenPrototypeWarning');

    // --- INÍCIO DA MODIFICAÇÃO (CHECA O TAMANHO DA TELA) ---
    // 2. Verifica se a tela é mobile (usando a mesma largura do seu menu: 1024px)
    const isMobile = window.matchMedia("(max-width: 1024px)").matches;
    // --- FIM DA MODIFICAÇÃO ---


    // 3. Se NÃO VIU e ESTIVER NO MOBILE, mostra o modal
    if (!hasSeenWarning && isMobile) { // <-- CONDIÇÃO ATUALIZADA
        // Adiciona a classe 'active' (do seu CSS) para mostrar o modal
        overlay.classList.add('active');
        // Adiciona a classe 'game-modal-open' (do seu CSS) para travar o scroll
        document.body.classList.add('game-modal-open');
    }

    // 4. Função para fechar o modal (esta função não muda)
